...
function openLesson(lessonId) {
  ...
  box.innerHTML = `<div class="flex justify-between items-start gap-3"><div><div class="text-xs text-indigo-600 font-bold">Leçon de français</div><h3 class="text-xl font-bold text-slate-800 font-heading">${lesson.title}</h3></div><button onclick="closeLesson()" class="text-slate-400 text-xl">✕</button></div><p class="mt-3 bg-indigo-50 border border-indigo-100 rounded-2xl p-3 text-sm text-indigo-900"><strong>À retenir :</strong> ${lesson.rule}</p><p class="mt-4 text-sm font-bold text-slate-700">${lesson.prompt}</p><div class="grid gap-2 mt-3">${lesson.options.map(option => `<button type="button" class="lesson-option bg-slate-50 border-2 border-slate-200 rounded-xl p-3 font-bold text-slate-700" data-answer="${encodeURIComponent(option)}" onclick="answerLesson(this, decodeURIComponent(this.dataset.answer))">${option}</button>`).join('')}</div><div id="lesson-feedback" class="mt-3 min-h-[2.5rem] text-sm font-bold"></div>`;
